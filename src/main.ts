...
import TypescriptAxiosPublisher from "./publisher/TypescriptAxiosPublisher";
...
async function main() {
  ...
  const schemaFile = await FileService.readYML<OpenApiYML>(
    Constants.SCHEMA_FILE_PATH
  );
  ...
  await execute(
    `openapi-generator-cli validate -i ${Constants.SCHEMA_FILE_PATH}`
  );
  ...
  deploymentNames.forEach(async (deploymentName) => {
    switch (deploymentName) {
      case "kotlin":
        ...
        await KotlinPublisher.publish(
          kotlinDeployment.artifact,
          kotlinDeployment.group,
      ...
      case "java":
        ...
        await JavaPublisher.publish(
          javaDeployment.artifact,
          javaDeployment.group,
          schemaFile.info.version
        );
        break;
      case "spring":
        ...
        await SpringPublisher.publish(
          springDeployment.artifact,
          springDeployment.group,
          schemaFile.info.version
        );
        break;
      case "typescript-angular":
        core.notice(`Found TypeScript Angular deployment`);
        const typescriptAngularDeployment = schemaFile["x-deploy"][
          "typescript-angular"
        ] as NPMDeployment;
        core.notice(
          `TypeScript Angular package name: ${typescriptAngularDeployment.name}`
        );
        await AngularTypescriptPublisher.publish(
          typescriptAngularDeployment.name
        );
        break;
      case "typescript-axios":
        core.notice(`Found TypeScript Axios deployment`);
        const typescriptAxiosDeployment = schemaFile["x-deploy"][
          "typescript-axios"
        ] as NPMDeployment;
        core.notice(
          `TypeScript Angular package name: ${typescriptAxiosDeployment.name}`
        );
        await TypescriptAxiosPublisher.publish(typescriptAxiosDeployment.name);
        break;
      default:
